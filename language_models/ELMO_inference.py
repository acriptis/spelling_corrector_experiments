...
class ELMOLM(object):
    ...
    def __init__(self,
        ...
        with open(scores_vocab_path, 'r') as f:
            kenlm_scores = self._adjust_scores_by_kenlm(json.load(f))
            self.scores_of_elmo_vocab_by_kenlm = np.array(kenlm_scores)
    ...
    def _adjust_scores_by_kenlm(self, l: List[float]):
        return [i if i != l[0] else 0 for i in l]

...
class SegmentTransducer:
    ...
    @staticmethod
    def make_default_operation_costs(alphabet, allow_spaces=True):
        """
        sets cost for every replacement, insertion, deletion and transposition
        """

        # TODO make costs parameters adaptable?
        operation_costs = dict()

        DEFAULT_COST = 1.0
        # cost for dropping the symbol:
        # SYMBOL_DROP_COST = 1.0
        SYMBOL_DROP_COST = 0.75

        # cost of insertion
        SYMBOL_INSERT_COST = 0.7

        # cost of inserting the space (token split cost)
        SPACE_INSERT_COST = 0.69

        # cost for dropping spaces (tokens merge)
        # SPACE_DROP_COST = 1.0
        SPACE_DROP_COST = 0.9

        TRANSPOSITION_COST = 0.85

        operation_costs[""] = {c: SYMBOL_DROP_COST for c in list(alphabet) + [' ']}
        for a in alphabet:
            current_costs = {c: DEFAULT_COST for c in alphabet}
            current_costs[a] = 0.0
            current_costs[""] = SYMBOL_INSERT_COST
            if allow_spaces:
                current_costs[" "] = SPACE_DROP_COST
            operation_costs[a] = current_costs
        # транспозиции
        for a, b in itertools.permutations(alphabet, 2):
            operation_costs[a + b] = {b + a: TRANSPOSITION_COST}
        # пробелы
        if allow_spaces:
            operation_costs[" "] = {c: DEFAULT_COST for c in alphabet}
            operation_costs[" "][""] = SPACE_INSERT_COST
